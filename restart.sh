...
nohup bash -c "cd '$REPO_ROOT/apps/voice-engine' && exec '$VENV_UVICORN' src.main:app --reload --port 8000 2>&1 | grep -v '/api/v1/assessment/.*/status'" \
  > "$VOICE_LOG" 2>&1 &
VOICE_PID=$!
...
# Tail logs with colored prefixes (filter status polls for voice engine)
tail_with_prefix() {
  local logfile=$1 prefix=$2 color=$3
  tail -f "$logfile" 2>/dev/null | grep -v '/api/v1/assessment/.*/status' | while IFS= read -r line; do
